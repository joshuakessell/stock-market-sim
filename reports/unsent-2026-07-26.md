# Morning Investment Research — Sunday, July 26, 2026
*Window covered: Saturday, July 25, 2026 00:00 ET – Sunday, July 26, 2026 ~05:08 ET*

**Data-sourcing note (read before the rest):** for the sixth run in a row, every structured, machine-readable source this briefing normally relies on was unreachable — SEC EDGAR, `data.sec.gov`, ClinicalTrials.gov, the Yahoo Finance quote API, and the Coinbase/Kraken/crypto.com APIs all returned `403 Forbidden` / tunnel-connection-failed. Confirmed independently via direct `curl`, the proxy's own status endpoint (`connect_rejected`, "gateway answered 403 to CONNECT — policy denial"), and the repo's `fetch_quotes.py` (all three equity tickers and both crypto assets errored). `WebFetch` also failed again this run on a plain page (`403 Forbidden` on a Yahoo Finance quote page). `WebSearch` was the only working tool, so anything below is sourced from search-result snippets rather than a fetched primary filing or exchange API — and **no live, cross-checked price quote could be pulled for any equity or crypto asset today**, moot for fills regardless since markets are closed (Sunday, and Saturday before it). See the Portfolio section and the administrative note at the end.

# 📈 Stocks

**No significant catalysts across the tracked themes (medical, technology/AI/quantum, space, emerging markets, deals, ETFs) cleared the bar in this window.** SEC EDGAR does not receive new 8-K/6-K filings over a weekend, so the primary signal channel was structurally empty regardless of the network block. Targeted searches for weekend and Sunday-night deal announcements, FDA actions, and Phase 3 readouts surfaced nothing dated inside the window that wasn't (a) outside the window (e.g., Vertex's $10B Crinetics acquisition — announced July 6; SoundHound/LivePerson and Paramount/WBD updates — dated Friday July 24, before this window opens), or (b) already logged in `seen_catalysts` from prior runs.

# ₿ Cryptos

**No material catalyst beyond ordinary price movement, and no verified price is available to report.** Search snippets describe Bitcoin holding a narrow range near $64,000–$64,400 and continued divergence between BTC (ETF outflows resuming July 23–24) and ETH (a fifth-plus straight day of ETF inflows) — but this is a continuation of catalysts already logged in `seen_catalysts` on July 24–25 (`BTC:2026-07-23:etf-outflow-streak-snapped`, `ETH:2026-07-24:etf-inflow-despite-price-drop`), not new information, and none of it is cross-checked against an exchange API this run. No snapshot table is included to avoid presenting unverified numbers as data.

---
**Sources consulted:** WebSearch only (SEC EDGAR, ClinicalTrials.gov, FDA.gov, Yahoo Finance quote API, and the Coinbase/Kraken/crypto.com APIs all blocked this session — 6th consecutive day, confirmed via `curl` and the repo's `fetch_quotes.py`). No news items found this run cleared the significance bar independently of that limitation.

*Informational only; not investment advice. Crypto is volatile and speculative. Verify independently before trading. Generated ~05:08 ET, July 26, 2026.*
...
## Snapshot
No live quotes were obtainable this run (see data-sourcing note above) — same limitation as the prior five runs. Today is also a Sunday (market closed), so no fills would have occurred regardless. As of the last confirmed state (July 20 fills; unchanged since):
- Cash: $7,397.92
...
|--------|--------|----------|----------------|-----------------|---------------|
| HUT | 11 | $100.53 | Unavailable (quote feed blocked); unverified search snippets suggest a $108–$118 range in recent days but are not used as an official mark | N/A | Intact — no new catalyst since the July 20 Beacon Point lease. |
| DYN | 63 | $23.75 | Unavailable (quote feed blocked); unverified search snippets suggest a $20–$24 range in recent days but are not used as an official mark | N/A | Watch (unchanged) — regulatory thesis intact; the now-larger $431.4M offering keeps closer scrutiny warranted than "intact." |

## Today's trades
**No trades today.** Markets are closed (Sunday), so no fills were possible regardless of findings. Separately, no reliable machine-readable quote existed for any ticker today — the same 403 policy block that has affected every run since around July 21 persisted for a sixth day. Nothing found this window cleared the Buy bar either — the window was structurally quiet (weekend, no new EDGAR filings) and search turned up nothing new and material. DYN and HUT were held unchanged.
...
`seen_catalysts` was reviewed for pruning (entries older than 30 days); the oldest entries are dated July 20, six days ago, so none qualify for pruning yet.

## Administrative notes
- `reports/unsent-2026-07-25.md` was found in the repo (yesterday's briefing was compiled but never actually sent, for the same reasons noted below). It has been renamed to `reports/morning-research-2026-07-25.md` in today's commit per the standing procedure, flagged here exactly once. Its content (Micron/SanDisk NAND-cycle reversal, Intel's Q2 beat vs. GAAP loss, the Microchip/Hailo deal, and the new China tariff) has not previously reached your inbox — it is appended in full below this report for reference, since none of it was superseded by anything found this run.
- **This is the sixth consecutive run** in which (a) the session's network egress policy blocks every structured data source (SEC EDGAR, ClinicalTrials.gov, FDA.gov, Yahoo Finance, all three crypto exchange APIs) with a 403, confirmed independently via curl, the proxy status endpoint, and the repo's own `fetch_quotes.py`, and (b) the Gmail connector available to this session exposes only draft creation with no send capability, meaning every run's report ends up as an unsent draft rather than delivered mail. Neither issue is a one-off; both are standing configuration gaps at the account/environment level. The paper-trading simulation has not executed a single fill or delivered a single email since it opened on July 20 — six days and six runs ago. This run's self-check (`scripts/selfcheck.py`) passed every ledger/cash/share-count check but failed the "quote present for every open position" check for the same network-block reason (DYN, HUT); no numbers were patched to force a pass.
...
# Appendix: July 25 briefing (previously unsent, included here for completeness)
